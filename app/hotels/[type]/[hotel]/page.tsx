...
function MasonryImageGrid() {
    const images = ["/hotels/Rectangle 22.png", "/hotels/Rectangle 18.png", "/hotels/Rectangle 17.png", "/hotels/Rectangle 8.png", "/hotels/Rectangle 21.png", "/hotels/Rectangle 23.png", "/hotels/Rectangle 20.png", "/hotels/Rectangle 19.png"];
    return (
        ...
        if (hasHalfStar) {
            stars.push(
                <div key="half" className="relative">
                    ...
                    <Star className="w-4 h-4 fill-orange-400 text-orange-400 absolute top-0 left-0 overflow-hidden"
            ...
            <div className="py-8 max-w-[1400px]">
                <div className="flex flex-col lg:flex-row gap-10">
                    <div className="flex-1">
                        ...
                        <div className="py-8">
                            <div className="text-[18px] font-semibold tracking-wider">Top Facilities</div>
                            <div className="grid grid-cols-2 gap-2 mt-4">
                                {facilities.map((facility, i) => (
                                    <div className="flex gap-2" key={i}>
                                        <span className="text-red">{facility.icon}</span>
                                        <span className="text-gray-700 text-[15px]">{facility.name}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
...
const HotelRoom = () => {
    return <div className="relative h-80 md:h-100 rounded-lg overflow-hidden shadow-lg group transition-transform hover:scale-105 cursor-pointer">
        ...
        <div className="absolute bottom-0 w-full bg-red text-white p-4 flex flex-row justify-between gap-1 items-center">
            <div>
                <h3 className="text-md md:text-lg tracking-widest mb-4">Standard twin ben, Multiple beds</h3>
                <div className="grid grid-cols-1 gap-2">
                    <div className="flex gap-2 text-sm">
                        <span><BriefcaseBusiness className="w-5" /></span>
                        <span>300 sq ft</span>
                    </div>
                    <div className="flex gap-2 text-sm">
                        <span><Wind className="w-5" /></span>
                        <span>Sleeps 3</span>
                    </div>
                    <div className="flex gap-2 text-sm">
                        <span><ThumbsUp className="w-5" /></span>
