...
const FormikPlaceArray: React.FC<FormikPlaceArrayProps> = ({
  ...
  const { errors, touched, initialValues } = useFormikContext<any>();

  const initialPlaceIds = new Set(
    initialValues[name]?.map((place: any) => place.editedPackagePlaceID) || []
  );
        ...
        {({ push, remove, form }) => (
          <div>
            {form.values[name]?.map((item: any, idx: number) => {
              const isInitialPlace =
                isEdit && initialPlaceIds.has(item.editedPackagePlaceID);

              return (
                <div key={idx}>
                  <div className="border border-gray-400 rounded p-3 mb-2">
            ...
            <Button
              ...
              onClick={() =>
                push({
                  ...
                  day_no: 1,
                  events: [""],
                  editedPackagePlaceID: "",
                })
              }
              className="bg-green-500 text-white text-sm px-2 py-1 rounded mb-4"
            />
          </div>
        )}
      </FieldArray>
